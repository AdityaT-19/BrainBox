...
class ProjectUpdateFormController extends GetxController {
  ProjectUpdateFormController(this.project);
  ...
  void onInit() {
    ...
    super.onInit();
    projectFeatures.addAll(project.features);
    projectTechnologies.addAll(project.technologies);
  }
  ...
  Future<void> submitForm() async {
    if (formKey.currentState!.validate()) {
      ...
      if (validateFeatures() && validateTechnologies() && validateGroupId()) {
        ...
        } else if (projectReportingFile.value != null) {
          final fileName = '$pid/report.pdf';
          await supabase.storage.from('projectReports').update(
                fileName,
                projectReportingFile.value!,
              );
          final url = supabase.storage.from('projectReports').getPublicUrl(
